...
AABBtree::~AABBtree()
{
    ...
    while (!Q.empty())
    {
        auto p = Q.front();
        Q.pop();
        if (p != nullptr && p->child1 != nullptr)
            Q.push(p->child1);
        if (p != nullptr && p->child2 != nullptr)
            Q.push(p->child2);
        delete p;
    }
}
...
{
    if (st == nullptr || st->collisionsChecked || st->isLeaf)
        return;
...
void AABBtree::collisionCheck(AABBnode *st1, AABBnode *st2, std::vector<std::pair<int, int>> &v)
{
    ...
    if (st1->isLeaf)
    {
        ...
        {
            if (st2->child1->box.collides(st1->box))
                collisionCheck(st2->child1, st1, v);
            if (st2->child2->box.collides(st1->box))
                collisionCheck(st2->child2, st1, v);
        }
    }
    else
    {
        if (st2->isLeaf)
        {
            if (st1->child1->box.collides(st2->box))
                collisionCheck(st1->child1, st2, v);
            if (st1->child2->box.collides(st2->box))
                collisionCheck(st1->child2, st2, v);
        }
        else
        {
            if (st1->child1->box.collides(st2->child1->box))
                collisionCheck(st1->child1, st2->child1, v);
            if (st1->child2->box.collides(st2->child2->box))
                collisionCheck(st1->child2, st2->child2, v);
            if (st2->child2->box.collides(st1->child1->box))
                collisionCheck(st2->child2, st1->child1, v);
            if (st2->child1->box.collides(st1->child2->box))
                collisionCheck(st2->child1, st1->child2, v);
        }
    }
...
void AABBtree::boxCollidersHelper(AABB *Box, AABBnode *st2, std::vector<int> &v)
{
    ...
    {
        if (st2->child1->box.collides(*Box))
            boxCollidersHelper(Box, st2->child1, v);
        if (st2->child2->box.collides(*Box))
            boxCollidersHelper(Box, st2->child2, v);
    }
}
